...
async function native_alpha(projectName, bundleId, branch) {
  output.info(
  ...
  const hostedInfo = hostedGitInfo.fromUrl(
    "https://github.com/hmdevelop/react-native-starter#alpha"
  );
  const url = hostedInfo.https({ noCommittish: true, noGitPlus: true });

  await execa("git", ["clone", url, projectName, "--branch", branch], {
    stdio: "inherit"
  });
